...
class CheckoutSession:
        ...
        try:
            ...
            if r2_res:
                ...
                try:
                    ...
                    if m_lines:
                        line = m_lines[0]
                        for key in ('totalPrice', 'expectedTotalPrice', 'subtotalPrice',
                                    'totalPriceAfterDiscounts'):
                            cand = line.get(key)
                            if cand and isinstance(cand, dict):
                                amt = cand.get('value', {}).get('amount')
                                if amt is not None:
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        # --- Re-read the exact server-confirmed values from the last proposal ---
        # Shopify recomputes merchandise and delivery line details on every
        # negotiation. Using cached step-2 values causes MERCHANDISE_EXPECTED_PRICE_MISMATCH
        # and DELIVERY_DELIVERY_LINE_DETAIL_CHANGED on low-priced items.
        submit_merch_total = self.merch_total
        submit_delivery_line = None
        submit_running_total = self.running_total
        submit_tax_amount = str(self.tax_amount)
        submit_shipping_amount = str(self.shipping_amount)

        if self.last_proposal_response:
            try:
                sp = self.last_proposal_response.get('sellerProposal', {}) or {}

                # --- merchandise expected total ---
                ...
                # --- delivery line (use the server's own selected strategy) ---
                delivery_data = sp.get('delivery', {}) or {}
                if delivery_data.get('__typename') == 'FilledDeliveryTerms':
                    d_lines = delivery_data.get('deliveryLines') or []
                    if d_lines:
                        submit_delivery_line = d_lines[0]

                # --- running total ---
                rt = sp.get('runningTotal')
                if rt:
                    rt_val = rt['value']['amount']
                    submit_running_total = str(
                        Decimal(str(rt_val)).quantize(
                            Decimal('0.01'), rounding=ROUND_HALF_UP
                        )
                    )
                ...
                if tax_data.get('__typename') == 'FilledTaxTerms':
                    ...
                    if tax_val is not None:
                        submit_tax_amount = str(
                            Decimal(str(tax_val)).quantize(
                                Decimal('0.01'), rounding=ROUND_HALF_UP
                            )
                        )
            except Exception:
        ...
        # Fallback for merchandise total
        if not submit_merch_total:
            submit_merch_total = str(
                Decimal(subtotal).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            )

        # --- Build the delivery line for submit ---
        if submit_delivery_line:
            # Reconstruct the delivery line exactly as the server returned it,
            # but only keep the fields the submit mutation accepts.
            selected = submit_delivery_line.get('selectedDeliveryStrategy', {}) or {}
            ...
            # If the server returned options for the selected strategy, pass them through.
            if submit_strategy_options:
