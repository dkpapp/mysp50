...
class CheckoutSession:
    ...
    def __init__(self, cc_info, site_url, explicit_variant_id, proxy_str):
        ...
        self.last_sync_payload = None           # exact JSON sent by _sync_final_proposal
        self.last_sync_seller_proposal = None   # sellerProposal from that response
    ...
    async def _execute_negotiations(self, session, graphql_url, subtotal):
        ...
        base_json = {
            ...
            'variables': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        'destination': {
                            'streetAddress': {          # was partialStreetAddress
        ...
        if self.delivery_strategy:
            ...
            dl['destinationChanged'] = False   # step 1 already set the address
        else:
    ...
    async def _sync_final_proposal(self, session, graphql_url):
        ...
        try:
            ...
            if rt:
                self.running_total = str(
                    Decimal(str(rt['value']['amount'])).quantize(
                        ...
                        # Refresh checkout total — this is what the payment amount
            # must equal on submit; runningTotal is only the pre-shipping total.
            ct = sp.get('checkoutTotal')
            if ct:
                self.checkout_total = str(
                    Decimal(str(ct['value']['amount'])).quantize(
                        Decimal('0.01'), rounding=ROUND_HALF_UP
                    )
                )
            ...
            # CRITICAL: the delivery handle ROTATES on every proposal response.
            # The sync we just sent carried the previous handle; the server
            # responded with a NEW one. Submit must send that NEW handle, or the
            # server compares it against the line it just stored and rejects with
            # DELIVERY_DELIVERY_LINE_DETAIL_CHANGED. Patch the payload we're
            # about to stash so submit picks up the fresh handle.
            if self.delivery_strategy and sync_json['variables']['delivery']['deliveryLines']:
                sync_json['variables']['delivery']['deliveryLines'][0][
                    'selectedDeliveryStrategy'
                ]['deliveryStrategyByHandle']['handle'] = self.delivery_strategy

            # Stash the exact payload and server proposal so _submit_payment can
            ...
            try:
                ...
                pass
            # --- END DIAGNOSTIC ---
                        # --- DIAGNOSTIC: log prices the server confirmed ---
            try:
                rt = sp.get('runningTotal', {}) or {}
                rt_amt = rt.get('value', {}).get('amount') if isinstance(rt, dict) else None
                tax = sp.get('tax', {}) or {}
                tax_amt = (tax.get('totalTaxAmount', {}) or {}).get('value', {}).get('amount')
                merch = sp.get('merchandise', {}) or {}
                m_lines = merch.get('merchandiseLines', []) or []
                m_amt = None
                if m_lines:
                    ta = m_lines[0].get('totalAmount') or {}
                    m_amt = ta.get('value', {}).get('amount')
                logger.warning(
                    "SYNC prices: runningTotal=%s checkoutTotal=%s tax=%s merch=%s | our cached: runningTotal=%s checkoutTotal=%s tax=%s merch=%s",
                    rt_amt, (sp.get('checkoutTotal', {}) or {}).get('value', {}).get('amount'),
                    tax_amt, m_amt,
                    self.running_total, self.checkout_total, self.tax_amount, self.merch_total,
                )
            except Exception:
                pass
            # --- END DIAGNOSTIC ---
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        # Reuse the sync's payload verbatim for everything except the payment
        # block (which needs the freshly-tokenized card). If the sync's payload
        # isn't available, fall back to a fresh-build path.
        if not self.last_sync_payload:
            return False, "No sync payload to submit against", ""

        sync_input = self.last_sync_payload['variables']

        # Deep-copy the delivery block so we can inject the fresh handle and
        # strip destination without mutating the cached sync payload.
        submit_delivery = json.loads(json.dumps(sync_input['delivery']))

        if submit_delivery.get('deliveryLines'):
            _dl = submit_delivery['deliveryLines'][0]

            # The destination is REQUIRED — without it the server rejects with
            # DESTINATION_ADDRESS_REQUIRED. Rebuild it in case the sync used a
            # partial address shape.
            _dl['destination'] = {
                'streetAddress': {
                    'address1': self.street, 'address2': '', 'city': self.city,
                    'countryCode': self.country, 'postalCode': self.s_zip,
            ...
            # The handle rotates on every proposal; use the freshest one.
            if self.delivery_strategy:
                _dl.setdefault('selectedDeliveryStrategy', {}).setdefault(
                    'deliveryStrategyByHandle', {})['handle'] = self.delivery_strategy

            # availableOn is present in the stored line but missing from our
            # submit. Add it — it's the only non-id, non-groupType scalar we
            # haven't yet sent.
            _dl.setdefault('availableOn', 'ASAP')

            # The final submit declares the destination as freshly changed so
            # the server re-validates the line against the current session state.
            _dl['destinationChanged'] = True
        ...
        sub_vars = {
            'input': {
                'sessionInput': sync_input['sessionInput'],
                'queueToken': self.queueToken or sync_input.get('queueToken') or '',
                'discounts': sync_input['discounts'],
                # These three are the ones the server compares strictly:
                'delivery': submit_delivery,
                'merchandise': sync_input['merchandise'],
                'payment': payment_block,
                # Everything else copied straight through:
                'buyerIdentity': sync_input['buyerIdentity'],
                'taxes': sync_input['taxes'],
                'tip': sync_input['tip'],
                'note': sync_input['note'],
                'localizationExtension': sync_input['localizationExtension'],
                'nonNegotiableTerms': sync_input['nonNegotiableTerms'],
                'optionalDuties': sync_input['optionalDuties'],
            },
        ...
        if self.checkpoint_data:
            sub_vars['input']['checkpointData'] = self.checkpoint_data
        ...
        # --- DIAGNOSTIC (kept until this works end-to-end) ---
        try:
            logger.warning(
                "SUBMIT delivery     = %s",
                json.dumps(sub_vars['input']['delivery']['deliveryLines'], sort_keys=True),
            )
            logger.warning(
                "SUBMIT merchandise  = %s",
                json.dumps(sub_vars['input']['merchandise']['merchandiseLines'], sort_keys=True),
            )
            logger.warning(
                "SYNC   delivery     = %s",
                json.dumps(sync_input['delivery']['deliveryLines'], sort_keys=True),
            )
        except Exception:
            pass
        # --- END DIAGNOSTIC ---
        ...
        # --- DIAGNOSTIC: log raw response whenever submit is not a clean success ---
        if '"submitForCompletion"' not in text and '"errors"' not in text:
            logger.warning("SUBMIT raw (pre-parse) = %s", text[:2500])
        # --- END DIAGNOSTIC ---
        ...
        try:
            ...
            if not sub_data:
                # --- DIAGNOSTIC: log raw response so we can see the real error ---
                logger.warning("SUBMIT raw response = %s", text[:2000])
                logger.warning("SUBMIT parsed r_json = %s", json.dumps(r_json)[:2000])
                # --- END DIAGNOSTIC ---
                ...
                if errors:
                    for error in errors:
                        ...
                        if code:
                            return False, code, ""
                        # No code — this is a GraphQL validation error.
                        # Surface its message instead of swallowing it.
            ...
            if rtype == 'SubmitRejected':
                try:
                    for e in sub_data.get('errors', []):
                        logger.warning(
                            "SubmitRejected violation: code=%s from=%s to=%s target=%s msg=%s",
                            e.get('code'),
                            json.dumps(e.get('from')),
                            json.dumps(e.get('to')),
                            json.dumps(e.get('target')),
                            e.get('localizedMessage') or e.get('nonLocalizedMessage'),
                        )
                except Exception:
                    pass
