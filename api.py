...
class CheckoutSession:
    ...
    def __init__(self, cc_info, site_url, explicit_variant_id, proxy_str):
        ...
        self.confirmed_delivery_id = None
        ...
        try:
            ...
            if delivery_data.get('__typename') == 'FilledDeliveryTerms':
                ...
                if d_lines:
                    ...
                    self.confirmed_delivery_id = d_lines[0].get('id')
            ...
            try:
                ...
                logger.warning(
                    "SYNC prices: runningTotal=%s tax=%s merch=%s | our cached: runningTotal=%s tax=%s merch=%s",
                    rt_amt, tax_amt, m_amt,
                    self.running_total, self.tax_amount, self.merch_total,
                )
            except Exception:
                pass
            # --- END DIAGNOSTIC ---
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        ...
        sync_input = self.last_sync_payload['variables']
                # Deep-copy the delivery block so we can inject the server-side line id
        # and the fresh handle without mutating the cached sync payload.
        ...
        if submit_delivery.get('deliveryLines'):
            ...
            # The server assigned this id when it first stored the line.
            # Submit must reference that same line by id or the server cannot
            # match our input to the stored delivery line and reports
            # DELIVERY_DELIVERY_LINE_DETAIL_CHANGED (from == to).
            if self.confirmed_delivery_id:
                _dl['id'] = self.confirmed_delivery_id
