...
class CheckoutSession:
    ...
    def __init__(self, cc_info, site_url, explicit_variant_id, proxy_str):
        ...
        self.attempt_token = ""
        self.sst = ""
        self.queueToken = None # Initialized gracefully for strictly nullable schemas
        self.stableId = "1"
        self.merch = ""
        self.build_id = None
        ...
        self.delivery_line_id = None  # Capture dynamically assigned server Delivery Line IDs
    ...
    async def run(self, session: aiohttp.ClientSession):
        ...
        self.queueToken = extract_between(text, 'queueToken&quot;:&quot;', '&quot;') or extract_between(text, '"queueToken":"', '"') or None
    ...
    async def _execute_negotiations(self, session, graphql_url, subtotal):
        ...
        # Sanitize variables to bypass 'INVALID_VARIABLE' schema requirements
        base_json = {
            'query': QUERY_PROPOSAL_SHIPPING,
            'variables': {
                'sessionInput': {'sessionToken': self.sst},
                'queueToken': self.queueToken,
                'changesetTokens': [], # Strict compliance for declared nullable arrays
                'discounts': {'lines': [], 'acceptUnexpectedDiscounts': True},
                'delivery': {
                    'deliveryLines': [{
                        'destination': {
                            'partialStreetAddress': {
                                'address1': self.street, 'address2': None, 'city': self.city, # None mapping prevents String! nullation issues over ""
                ...
                'payment': {
                    ...
                    'billingAddress': {
                        'streetAddress': {
                            'address1': self.street, 'address2': None, 'city': self.city, 
                            'countryCode': self.country, 'postalCode': self.s_zip,
                            'firstName': self.firstName, 'lastName': self.lastName,
                            'zoneCode': self.state, 'phone': self.phone
                        }
                ...
                'buyerIdentity': {
                    ...
                    'shopPayOptInPhone': {'number': self.phone, 'countryCode': self.country}, # Added number mapping securely
                    'rememberMe': False
                },
                'tip': {'tipLines': []},
                'taxes': {
        ...
        if 'errors' in resp_json and resp_json['errors']:
            ...
            for e in resp_json['errors'][:3]:
                ...
                # Intercept fragment errors so the user is not confused by static schema mismatches
                if "useAndDefineFragment" in msg or "Fragment" in msg:
                    error_msgs.append("GraphQL Schema Mismatch (Site API Update)")
                else:
        ...
        try:
            ...
            result = data_block.get('session', {}).get('negotiate', {}).get('result', {})
            ...
            if delivery_data.get('__typename') == 'FilledDeliveryTerms':
                ...
                if delivery_lines and len(delivery_lines) > 0:
                    # Dynamically capture the resolved delivery line ID to prevent Update Violations
                    self.delivery_line_id = delivery_lines[0].get('id')
        ...
        # Inject newly received tracking tokens backward into payload variables
        if self.checkpoint_data: 
            base_json['variables']['checkpointData'] = self.checkpoint_data
            
        dl = base_json['variables']['delivery']['deliveryLines'][0]
        
        # Safely bind updates to the specific server-resolved Delivery Line
        if self.delivery_line_id:
            dl['id'] = self.delivery_line_id
            
        # Bind strategy condition mapping fallback to omit invalid "" strategy string errors
        ...
        # Switch to lenient constraints to bypass ApplyChangeViolation (DELIVERY_DELIVERY_LINE_DETAIL_CHANGED)
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        # Build the dynamic delivery line utilizing the resolved constraints
        delivery_line_submit = {
            'destination': {
                'streetAddress': {
                    'address1': self.street, 'address2': None, 'city': self.city,
                    'countryCode': self.country, 'postalCode': self.s_zip,
            ...
            'targetMerchandiseLines': {'any': True},
            'deliveryMethodTypes': ['SHIPPING'],
            'expectedTotalPrice': {'any': True}, # Ensure variable flexibility matches the dynamically calculated amount
            'destinationChanged': False
        }
        
        # Inject the ID to bypass DELIVERY_DELIVERY_LINE_DETAIL_CHANGED
        if self.delivery_line_id:
            delivery_line_submit['id'] = self.delivery_line_id
            
        if self.delivery_strategy:
            delivery_line_submit['selectedDeliveryStrategy'] = {
                'deliveryStrategyByHandle': {'handle': self.delivery_strategy, 'customDeliveryRate': False},
        ...
        sub_vars = {
            'input': {
                ...
                'discounts': {'lines': [], 'acceptUnexpectedDiscounts': True},
                'delivery': {
                    'deliveryLines': [delivery_line_submit],
        ...
        try:
            ...
            # The safe mapping here prevents "'NoneType' object has no attribute 'get'" exceptions 
            # while accurately fetching un-coded errors if Shopify returns 'null' data blocks.
            data_block = r_json.get('data')
            if data_block is None:
                data_block = {}
                
            sub_data = data_block.get('submitForCompletion')
                ...
                if errors:
                    for error in errors:
                        ...
                        if code: 
                            return False, code, ""
                        if msg: 
                            # Safe abstraction over static schema mismatch errors
                            if "useAndDefineFragment" in msg or "Fragment" in msg:
                                return False, "GraphQL Schema Mismatch (Site API Update)", ""
                            return False, msg, ""
