...
def extract_clean_response(message):
    """Optimized string extraction using regex. Increased limit to prevent truncation errors."""
    ...
    for pattern in CLEAN_PATTERNS:
        for match in pattern.findall(message):
            ...
            # Increased truncation limit to 100 to prevent GraphQL error texts from being truncated 
            # and resulting in "unexpected end of fil" visual bugs in your front-end
            if match and "_" in match and len(match) < 100:
                return match.strip("{}:'\" ")
...
async def fetch_products(domain: str, proxy_str: Optional[str], session: aiohttp.ClientSession) -> Union[dict, Tuple[bool, str]]:
    """Fetch products and find the absolute cheapest product under $10."""
...
class CheckoutSession:
    ...
    async def _execute_negotiations(self, session, graphql_url, subtotal):
        ...
        try:
            # FIX: Used `.get(...) or {}` to prevent AttributeError when JSON value is explicitly `null`
            data_block = resp_json.get('data') or {}
            session_block = data_block.get('session') or {}
            negotiate_block = session_block.get('negotiate') or {}
            result = negotiate_block.get('result') or {}
            ...
            seller_proposal = result.get('sellerProposal') or {}
            ...
            delivery_data = seller_proposal.get('delivery') or {}
        ...
        try:
            ...
            final_total_data = seller_proposal.get('checkoutTotal') or seller_proposal.get('runningTotal')
            if final_total_data:
                self.running_total = final_total_data['value']['amount']
                self.total_price = str(self.running_total)

            final_delivery = seller_proposal.get('delivery') or {}
            if final_delivery.get('__typename') == 'FilledDeliveryTerms':
                for dline in final_delivery.get('deliveryLines', []):
                    strats = dline.get('availableDeliveryStrategies', [])
                    for strat in strats:
                        if strat.get('handle') == self.delivery_strategy:
                            try:
                                amt = strat.get('amount', {}).get('value', {}).get('amount')
                                if amt: self.shipping_amount = float(amt)
                            except: pass
                            break
                            
            final_tax = seller_proposal.get('tax') or {}
            if final_tax.get('__typename') == 'FilledTaxTerms':
                try:
                    amt = final_tax.get('totalTaxAmount', {}).get('value', {}).get('amount')
                    if amt: self.tax_amount = float(amt)
                except: pass

        except Exception as e:
            logger.warning(f"Fallback warning in execute_negotiations Step 2: {e}")
            pass 
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        ...
        try:
            ...
            # FIX: Replaced deep getter with chained .get() or {} implementation to gracefully handle API errors 
            data_block = r_json.get('data') or {}
            sub_data = data_block.get('submitForCompletion') or {}
            
            if not sub_data:
                errors = r_json.get('errors') or []
                if errors:
                    err_msgs = []
                    for error in errors:
            ...
            return False, f"Unknown Response Type: {rtype}", ""
        except Exception as e:
            logger.error(f"Error parsing submit response: {e}")
    ...
    async def _poll_receipt(self, session, graphql_url, rid):
        ...
        for _ in range(4):
            ...
            try:
                ...
                # FIX: Implement explicit .get() fallback safety checks matching other routines 
                data_block = final_json.get('data') or {}
                rdata = data_block.get('receipt') or {}
                ...
                logger.debug(f"JSON polling exception: {e}")
                pass
        ...
        try:
            ...
            receipt = data_block.get('receipt') or {}
            processing_error = receipt.get('processingError') or {}
            err_code = processing_error.get('code')
