...
class CheckoutSession:
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        ...
        if submit_delivery.get('deliveryLines'):
            ...
            # Strip destination — the server's stored line has no such field.
            _dl.pop('destination', None)

        # The merchandise expectedTotalPrice must be the tax-inclusive line
        # total for submit — checkoutTotal - shipping. Using the pre-tax merch
        # total triggers "Your order total has changed."
        try:
            merch_block = sync_input['merchandise']
            if merch_block.get('merchandiseLines'):
                _ml = merch_block['merchandiseLines'][0]
                try:
                    line_total = (
                        Decimal(self.checkout_total) - self.shipping_amount
                    ).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                except Exception:
                    line_total = Decimal(self.checkout_total)
                _ml['expectedTotalPrice'] = {'value': {
