...
class CheckoutSession:
    ...
    async def run(self, session: aiohttp.ClientSession):
        ...
        if not sync_ok: return False, sync_msg, self.gateway, self.total_price, self.currency
    ...
    async def _sync_final_proposal(self, session, graphql_url):
        ...
        try:
            ...
            # Refresh checkpointData — critical for submit
            ...
            # Refresh merchandise total
            merch_data = sp.get('merchandise', {}) or {}
            m_lines = merch_data.get('merchandiseLines') or []
            if m_lines:
                line = m_lines[0]
                for key in ('totalPrice', 'expectedTotalPrice', 'subtotalPrice', 'totalPriceAfterDiscounts'):
                    cand = line.get(key)
                    if cand and isinstance(cand, dict):
                        amt = cand.get('value', {}).get('amount')
                        if amt is not None:
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        ...
        submit_merch_total = self.merch_total or str(
