...
class CheckoutSession:
    ...
    def __init__(self, cc_info, site_url, explicit_variant_id, proxy_str):
        ...
        self.shipping_amount_str = "0.00" # Explicit string preservation to bypass floating point mismatch
        self.tax_amount = 0.0
        self.tax_amount_str = "0.00"
    ...
    async def _execute_negotiations(self, session, graphql_url, subtotal):
            ...
            if delivery_data.get('__typename') == 'FilledDeliveryTerms':
                ...
                if delivery_lines:
                    ...
                    if available_strategies:
                        ...
                        # Explicit string extraction to prevent float->string formatting differences
                        self.shipping_amount_str = available_strategies[0].get('amount', {}).get('value', {}).get('amount', '0.00')
                        try:
                            self.shipping_amount = float(self.shipping_amount_str)
                        except: self.shipping_amount = 0.0

            tax_data = seller_proposal.get('tax', {})
            if tax_data.get('__typename') == 'FilledTaxTerms':
                self.tax_amount_str = tax_data.get('totalTaxAmount', {}).get('value', {}).get('amount', '0.00')
                try:
                    self.tax_amount = float(self.tax_amount_str)
        ...
        # Explicit inputs to satisfy strict GraphQL schemas
        dl['targetMerchandiseLines'] = {'lines': [{'stableId': self.stableId}]}
        
        # Use exact extracted string format to avoid ApplyChangeViolations
        dl['expectedTotalPrice'] = {'value': {'amount': self.shipping_amount_str, 'currencyCode': self.currency}}
        ...
        base_json['variables']['taxes']['proposedTotalAmount']['value']['amount'] = self.tax_amount_str
    ...
    async def _submit_payment(self, session, graphql_url, token, subtotal):
        # Build strict, type-safe GraphQL inputs utilizing precise cached string values
        sub_vars = {
            'input': {
                'sessionInput': {'sessionToken': self.sst},
                'queueToken': self.queueToken,
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        'deliveryMethodTypes': ['SHIPPING'],
                        'expectedTotalPrice': {'value': {'amount': self.shipping_amount_str, 'currencyCode': self.currency}},
                ...
                'taxes': {
                    'proposedAllocations': None,
                    'proposedTotalAmount': {'value': {'amount': self.tax_amount_str, 'currencyCode': self.currency}},
        ...
        try:
            ...
            # Enhanced GraphQL Error Extraction
            if not sub_data:
                ...
                if errors:
                    ...
                    for error in errors:
                        ...
                        if code and msg:
                            ...
                            err_msgs.append(msg)
                    return False, f"GraphQL Error: {'; '.join(err_msgs)[:150]}", ""
                    
                return False, f"Empty submit response: {text[:80]}", ""
